...
namespace flick {  
  begin_test_case(normalized_scattering_matrix_fit_test) {
    ...
    write(m_fit.scattering_scaling_factor(),"material/ab_functions/scaling_factor.txt");
    double p = 1;
    for (size_t i=0; i<4; ++i) {
      write(pl_function{x,a[i]},"material/ab_functions/a"+std::to_string(i)+".txt",15);
      write(pl_function{x,m_fit.scaled_a(i)},"material/ab_functions/a_scaled"+std::to_string(i)+".txt",15);
      write(pl_function{x,m_fit.fitted_scaled_a(i)},"material/ab_functions/a_scaled_fitted"+std::to_string(i)+".txt",15);
    ...
    for (size_t i=0; i<2; ++i) {
      ...
      write(pl_function{x,m_fit.scaled_b(i)},"material/ab_functions/b_scaled"+std::to_string(i)+".txt",15);
      write(pl_function{x,m_fit.fitted_scaled_b(i)},"material/ab_functions/b_scaled_fitted"+std::to_string(i)+".txt",15);
