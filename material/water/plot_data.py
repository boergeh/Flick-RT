...
os.system('flick list --xy ./absorption_coefficient.txt > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
ax[0,0].loglog(xy[:,0], xy[:,1],'b-')
ax[0,0].grid()
ax[0,0].set_ylabel("Absorption coefficient [m$^{-1}$]")

os.system('flick list --xy ./refractive_index.txt > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
...
os.system('flick list --xy ./temperature_correction.txt > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
...
os.system('flick list --xy ./salinity_correction.txt > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
