...
os.system('flick text ./absorption_coefficient.txt xy > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
ax[0,0].loglog(xy[:,0], xy[:,1],'b-')
ax[0,0].grid()
ax[0,0].set_ylabel("Absorption coefficient [m$^{-1}$]")

os.system('flick text ./refractive_index.txt xy > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
...
os.system('flick text ./temperature_correction.txt xy > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
...
os.system('flick text ./salinity_correction.txt xy > xy_tmp.txt')
xy = np.loadtxt("xy_tmp.txt")
