...
namespace flick {
  begin_test_case(boundary_test) {
    ...
    check_close(ui2.boundary_area(), 4*pi, 9);
    check_close(ui2.enclosed_volume(), 4./3*pi, 9);
    check_small(rms(ui2.center_of_gravity(),center), 0.2);
    ...
    check(b3.intersection(observer3).has_value()==false);
    check(b3.intersection(observer3.move_to({0,0,-9})).has_value()==true);
    check(b3.intersection(observer3.move_to({0,0,-0.1})).has_value()==true);
    auto ui3 = uniform_intersections{b3,2000,limits{3,3.2}};
    check_close(ui3.enclosed_volume(), 4./3*pi*pow(r1,3)/2, 9);
    double a = 4*pi*pow(r1,2)/2+pi*pow(r1,2);
    check_close(ui3.boundary_area(), a, 9);
    ...
    check_close(ui4.boundary_area(),pow(2*d,2)*6,9);
    check_close(ui4.enclosed_volume(),pow(2*d,3),9);
    check_small(rms(ui4.center_of_gravity(),center4),3);
    ...
    check_close(ui5.boundary_area(),4*pi/4+pi,9);
    check_close(ui5.enclosed_volume(),4./3*pi/4,9);
    ...
    check_close(ui6.boundary_area(),4*pi*(1+pow(0.5,2)),9);
    check_close(ui6.enclosed_volume(),4./3*pi*(1-pow(0.5,3)),9);

    cubical_boundary cb{1};
    cb.move_by({1,0,0});
    cb.rotate_by(rotation_about_z(pi/2),{-1,0,0});
    check_small(rms(cb.placement().position(),{-1,2,0}));
